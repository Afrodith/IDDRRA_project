...
void SteppingAction::UserSteppingAction(const G4Step* theStep)
{
    ...
    //G4cout << theStep << "  " << G4endl;
  {
    ...
    if ((edepStep > 0.) && (theVolume==targetVolume))
    {
      ...
      {
	//G4cout << x <<" "<< y <<" "<< z <<" "<< "energy  " << edepStep << G4endl;
    ...
    G4double edepStep)
{
  ...
  if (hit==1)
  {
    //cnt++;
    //G4cout << cnt;
    if ((intResidue==0)||(intResidue==1)/*||(intResidue==2)*/) //Edep in Phosphate or Sugar  //Base added for DnaDosimeter
    {
      fpEventAction->AddEdepToNucleotide(numStrand,numNucl,edepStep);
      //G4cout << "   	resID=" << intResidue << G4endl;
      return true;
    }
    else
    {
