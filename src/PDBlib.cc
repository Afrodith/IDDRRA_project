                        ...
                        unsigned short int verbose = 0)
{
  ...
  else
  {
    ...
    //double DBL_MAX = 5000;
    //cout <<endl<<endl<<nbMolecule<<" "<<DBL_MAX<<endl<<endl;
    minGlobZ = -DBL_MAX;
    minGlobX = -DBL_MAX;
    minGlobY = -DBL_MAX;
    maxGlobZ = DBL_MAX;
    ...
    while(!infile.eof())
    {
      ...
      else if((sLine.substr(0, 5)).compare("TER  ") == 0) //2 spaces
      {
        //////////////////////////////////////////
      ...
      else if((sLine.substr(0, 4)).compare("ATOM") == 0)
      {

        /************ Begin ATOM ******************/
        //serial
        istringstream((sLine.substr(5, 6))) >> serial;
...

        {
          ...
          //x,y,z :
          stringstream((sLine.substr(29, 9))) >> x;
          stringstream((sLine.substr(38, 9))) >> y;
          stringstream((sLine.substr(47, 9))) >> z;
          //occupancy :
          occupancy = 1.;
                                         ...
                                         double &tZ) //Translation for bounding volume
{
  ...
  std::cout << endl << "XLength= " << 2*dX <<"   YLength= " << 2*dY <<"   ZLength= " << 2*dZ;

  tX = minminX + (maxmaxX - minminX) / 2.;
  tY = minminY + (maxmaxY - minminY) / 2.;
  tZ = minminZ + (maxmaxZ - minminZ) / 2.;

  std::cout << "   XCenter= " << tX <<"   YCenter= " << tY <<"   ZCenter= " << tZ << endl << endl;
                                               ...
                                               int &codeResidue)
{
  ...
  while(moleculeListTemp)
  {
    ...
    while(residueListTemp)
    {
      ...
      {
        //G4cout<<x<<" "<<BarycenterList->fCenterX<<" "<<y<<" "<< BarycenterList->fCenterY<<" "<<z<<" "<<BarycenterList->fCenterZ<<" "<<numStrand<<" "<<distEdepDNA<<" "<<BarycenterList->GetRadius()<<G4endl;
	  ...
	  //G4cout<<x<<" "<<AtomTemp->GetX()<<" "<<y<<" "<<AtomTemp->GetY()<<" "<<z<<" "<<AtomTemp->GetZ()<<" "<<G4endl;
	  //G4cout<<"dEA="<<distEdepAtom<<" aR="<<AtomTemp->GetVanDerWaalsRadius()<<" sD="<<smallestDist<<G4endl;

	  
	  if((distEdepAtom < AtomTemp->GetVanDerWaalsRadius()) && (distEdepAtom < smallestDist))
          {
	    //G4cout<<"dEA="<<distEdepAtom<<" aR="<<AtomTemp->GetVanDerWaalsRadius()<<" sD="<<smallestDist<<G4endl<<G4endl;
  ...
  codeResidue = BSP;
  //G4cout << matchFound << "blablabla" << G4endl;
  return matchFound;
}
