  ...
  fCheckOverlaps(false)
{
  ...
  //
  fPdbFileName=G4String("python/dnaPython.pdb");
  fPdbFileStatus=0;
  fChosenOption=11;
...
{
  // Geometry parameters
  G4double worldSize  = 400000*1*nm;
...
/*  
   // Number of DNA
  G4Box* PhantomsS
  = new G4Box("Phantoms",           // its name
	      500*nm, 200*nm, 200*nm); // its size
    
  G4LogicalVolume* PhantomsLV = 
  new G4LogicalVolume(PhantomsS,fpDefaultMaterial,"Phantoms");

  //for (auto i=0;i<10;i++) {
  //    G4double x1 = (i-10/2)*1000.*nm;
  new G4PVPlacement(0,G4ThreeVector(-100.*nm,95.*nm,200.*nm),PhantomsLV,
                    "Phantoms",worldLV,
                     false,0,true);
  //}

  MyVisAtt_ZZ = new G4VisAttributes(G4Colour(10,0.,0.));   
  PhantomsLV->SetVisAttributes(MyVisAtt_ZZ);
*/
    ...
    unsigned short int option)
{
  ...
  //PhantomsLV=worldPV->GetLogicalVolume();
